...
static void dt_telemetry_sample_rate_handler(DX_DEVICE_TWIN_BINDING *deviceTwinBinding);
...
static DX_MESSAGE_PROPERTY *messageProperties[] = {&(DX_MESSAGE_PROPERTY){.key = "mode", .value = "autoTelemetry"}, &(DX_MESSAGE_PROPERTY){.key = "type", .value = "telemetry"},
...
static DX_DIRECT_METHOD_BINDING dm_light_control = {.methodName = "LightControl", .handler = TelemetryIntervalHandler};
